...
for max_d in 3 4 5; do
	for n_estim in 50 100 150; do
		for max_feat in 0.25 0.5 1; do
			...
			echo "P_LR=999999" >> hyperparams.py
...
EXP_NUM=0
echo "Num,Max Depth,n_estimators,LR,MSE,MAE" > search_results_rf_reg.csv
for max_d in 3 4 5; do
	for n_estim in 50 100 150; do
		for max_lr in 0.1 0.01 0.001; do
			echo "P_MAX_D=$max_d" > hyperparams.py
			echo "P_N_ESTIM=$n_estim" >> hyperparams.py
			echo "P_LR=$max_lr" >> hyperparams.py
			echo "EXP_NAME=$EXP_NUM" >> hyperparams.py

			# Placeholder for import
			echo "P_MAX_FEAT=999999" >> hyperparams.py
